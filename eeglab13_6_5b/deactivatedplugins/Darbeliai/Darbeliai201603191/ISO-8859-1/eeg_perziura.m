...
% eeg_perziura - EEG periros/lyginimo varikliukas
%
% eeg_perziura(EEG)
...
% epochuot duomen lang suvienodinimas
...
% epochuot ir neepochuot duomen susiejimas
if isfield(EEG1.event, 'urevent') && isfield(EEG2.event, 'urevent');
    ...
    if sutampa_ivykiai;
        ...
        sk=[EEG2.event(urid2_sutampa_i).laikas_ms] - [EEG1.event(urid1_sutampa_i).laikas_ms]; % sutampai vyki laik poslinkiai        
        if EEG1.trials <= 1;
            ...
            epochos_sutampanciu=epochos(urid1_sutampa_i); %[EEG1.event(urid1_sutampa_i).epoch]; % sutampani vyki epochos
            [~,uniq_ep_i]=unique(epochos_sutampanciu); % vyki indeksai, ties kuriais prasideda unikalios epochos
            urev=[];
            for ii=1:length(uniq_ep_i);
                i_nuo=uniq_ep_i(ii); % EEG1 ivykio, NUO kurio darbuojams, indeksas sutampani vyki srae
                if ii == length(uniq_ep_i);
                    i_iki=uniq_ep_i(end); % EEG1 ivykio, IKI kurio darbuojams, indeksas sutampani vyki srae
                ...
                %disp(['[xtk: ' num2str(ti1) ':' num2str(ti2) ']']);
...
if isequal(l1,l2);
    ...
    elseif iscellstr(l1) && (length(unique(l1)) == size(EEG1.data,1)) && ...
        ...
        %elseif EEG.urchanlocs
            ...
            % Papildomai pertvarkyti, bet tolesns 5 eiluts nebtinos
            [~,k2j]=ismember(l2,l);
...
if isempty(which('knnsearch'));
    ...
    [idx_,D_]=knnsearch(x2,x1_); % jei dubliuojasi x1, is variantas bus greitesnis
else
    warning('function "knnsearch" not found!');
    idx_=nan(size(x1_)); D_=idx_;
    l=length(x1_); %l=min(10000,l);
    % skaidyti, kad neviryt MATLAB kintamj dydio ribojim
    zgsn=5000; % sumaink, jei randi klaid: Out of memory. Type HELP MEMORY for your options.
    %tic;
...
if dim3 > 1;
    ...
    for i=0:dim3;
        l=0.5 + i * dim2 - i; %  ...- i dl to, kad terpiam NaN gale
...
% Pakeisti X aies centravim ir plot
try cx=g(1).x;
catch
    cx=LX(1);
end;
...
% reagavimas  klaviatros paspaudimus
...
try
    switch ck
        ...
        case {'downarrow' 'uparrow'} % Y ais apversta!
...
catch % Paprastai klaida bna, jei kompiuteris lagina
    return;
end;
try
    ...
    elseif ismember('alt',modifiers);
        ...
        lim_max=get(scrollHandles(asisN),[asisR 'lim']);
        lim_plt=(lim_dbr(2)-lim_dbr(1));
        if eventdata.VerticalScrollCount > 0; % Y ais ir taip apversta
...
% Shift   - leidia tsti ymjim net atleidus pels klavi iki kito spragteljimo
% Control - leidia atymti gretimas paymtas epochas
...
if ismember(xi,ni) && ~ismember('shift', modifiers)...
    ...
    %disp([EEG1.times(xi1) EEG1.times(xi2)] * 0.001) % jau paymtas intervalas ms
...
if ~ismember(get(gcf, 'SelectionType'),{'normal' 'extend' 'open'}); % su Shift arba be joki
    setappdata(a,'spragtelejimo_vieta',[]);
    return;
end;
...
% Lauti paymtos srities intervalus, sekundmis
laikas=[];
a=getappdata(gcf,'main_axes');
try
    if isempty(getappdata(a,'zymeti')); return; end;
