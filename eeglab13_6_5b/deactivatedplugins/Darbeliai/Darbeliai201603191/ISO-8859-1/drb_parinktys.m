...
% drb_parinktys - Darbeli lang parinki valdymas
%
...
%% Dialogas parinki rinkinio pavadinimui vesti
% sranka2 nurodoma eksportuojant kaip aptiktoji
vardas='';
komentaras='';
pavyko=0;
if isempty(saranka2);
...
if papildyti;
    try
        ...
        for persidengia_i2=persidengia_i2v;
            ...
            if (pavyko == 2) && ~strcmp(saranka(persidengia_i1).vardas, vardas2) ; % perrayta ir pervadinta
                persidengia_i2v=unique([persidengia_i2v find(ismember(esami2,{vardas2}))]);
                pavyko=1;
            end;
            if pavyko < 2; % neleista perrayti: ataukta arba pervadinta;
        ...
        if length(persidengia_i2v) == length(saranka2);
            ...
            saranka2=saranka2(setdiff(1:length(saranka2),persidengia_i2v)); % itrint persidengianius
    ...
    for dbr_i=1:10; dbr_id=num2str(dbr_i);
        try ci=ismember({saranka(i).parinktys.id}, {['checkbox_drb' dbr_id]});
            if saranka(i).parinktys(ci).Value;
                ...
                if length(d) >= 1 && length(p) >= 1; % buvo klaida pop_meta_drb, kai 2x septintus darbus ra ir nera atunt
                    d=d(1); p=p(1);
...
try
    ...
    for persidengia_i2=persidengia_i2v;
        ...
        if pavyko == 2; % perrayta ir pervadinta
