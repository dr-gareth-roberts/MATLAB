%
% Imanus pop_pervadinimas su galimybe atnaujinti EEGLAB duomen inormacij
%
...
% Jei nenurodytas filtras - aldyk jau pasirinktus failus
try 
...
try
    ...
    varargout{3} = varargout{3}(get(handles.listbox_siulomi,'Value')); % paymtos rinkmenos
catch err;
    varargout{1} = [];
    varargout{2} = '';
    varargout{3} = {};
...
% grina lentel, kurioje yra logins reikms.
% eiluts atitinka silom rinkmen pavadinim eilikum
% 1-as  stulpelis  ar toks failas jau yra
% 2-as  stulpelis  ar silomas failo pavadinimas sutampa su originaliu
% 3-ias stulpelis - ar silomas failas perrayt tok original fail, kur ketinate pervadinti
lentele=[];
...
for sablono_i=1:length(sablonai);
    ...
    set(handles.edit10,'String',sablonas{2}); % silomas (naujas) pavadinimas
    set(handles.edit5, 'String',sablonas{3}); % tiriamasis
    set(handles.edit6, 'String',sablonas{4}); % grup
    set(handles.edit7, 'String',sablonas{5}); % slyga
...
for i=1:Pasirinktu_failu_N;
    ...
    %Darbo_eigos_busena(handles, 'keliami duomenys...', DarboNr, i, Pasirinktu_failu_N);
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
i=get(handles.edit_tikri,'String'); % klimo
