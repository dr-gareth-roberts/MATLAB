% pop_RRI_perziura - EKG ir R-R interval perira, RRI koregavimas
% 
...
warning('off','MATLAB:modes:mode:InvalidPropertySet'); % pvz., 'brush' tempiant ant slinkikli
...
% Atstatyti
h25 = uimenu( 'Parent',handles.m_Duomenys, 'Accelerator','P', 'Callback',@(hObject,eventdata)pop_RRI_perziura('pushbutton_atstatyti_Callback',hObject,eventdata,guidata(hObject)), 'Label','Nuo pradi', 'Tag','Atstatyti');
% Importuoti
...
% Uverti neperduodant laik
h27 = uimenu( 'Parent',handles.m_Duomenys, 'Accelerator','X', 'Callback',@(hObject,eventdata)pop_RRI_perziura('figure1_CloseRequestFcn',hObject,eventdata,guidata(hObject)), 'Separator','on', 'Label','Uverti', 'Tag','Uzverti');
% Perduoti laikus ir uverti, jei atidaryta i kitos programos
...
% istorija_atgal
h42 = uimenu( 'Parent',handles.m_Taisa, 'Accelerator','Z', 'Callback',@(hObject,eventdata)pop_RRI_perziura('istorija_atgal_ClickedCallback',hObject,eventdata,guidata(hObject)), 'Label','Ataukti veiksm', 'Tag','istorija_atgal');
% istorija_tolyn
h43 = uimenu( 'Parent',handles.m_Taisa, 'Accelerator','Y', 'Callback',@(hObject,eventdata)pop_RRI_perziura('istorija_tolyn_ClickedCallback',hObject,eventdata,guidata(hObject)), 'Label','Grinti atauktj', 'Tag','istorija_tolyn');
% Naujas_dantelis
h32 = uimenu( 'Parent',handles.m_Taisa, 'Accelerator','R', 'Callback',@(hObject,eventdata)pop_RRI_perziura('naujas_dantelis',hObject,eventdata,guidata(hObject)), 'separator', 'on', 'Label','Pridti tak ranka', 'Tag','Naujas_dantelis');
% Salinti
h33 = uimenu( 'Parent',handles.m_Taisa, 'Accelerator','D', 'Callback',@(hObject,eventdata)pop_RRI_perziura('salinti_pazymetuosius1',hObject,eventdata,guidata(hObject)), 'Label','Paalinti paymtus takus', 'Tag','Salinti');
...
% Nejungti
handles.m_Nejungti = uimenu( 'Parent',handles.m_Rodymas, 'Callback',@(hObject,eventdata)pop_RRI_perziura('Nejungti_ClickedCallback',hObject,eventdata,guidata(hObject)), 'checked','on', 'Label','Nejungti ypa nutolusi tak', 'Tag','Nejungti');
% Uzribis
handles.m_Uzribis = uimenu( 'Parent',handles.m_Rodymas, 'Callback',@(hObject,eventdata)pop_RRI_perziura('edit_ribos_Callback',hObject,eventdata,guidata(hObject)), 'separator', 'on', 'Label','Paymti tartinus takus', 'Tag','Uzribis');
% toggle_brush
%h49 = uimenu( 'Parent',handles.m_Rodymas, 'Callback',' ', 'OnCallback',@(hObject,eventdata)pop_RRI_perziura('axes_rri_ButtonDownFcn',hObject,eventdata,guidata(hObject)), 'OffCallback',@(hObject,eventdata)pop_RRI_perziura('toggle_brush_OffCallback',hObject,eventdata,guidata(hObject)), 'Label','ymjimo apvedant staiakampiu veiksena', 'Tag','toggle_brush');
% aktyvusis
%h50 = uimenu( 'Parent',handles.m_Rodymas, 'Callback',' ', 'OnCallback',@(hObject,eventdata)pop_RRI_perziura('aktyvusis_OnCallback',hObject,eventdata,guidata(hObject)), 'OffCallback',@(hObject,eventdata)pop_RRI_perziura('aktyvusis_OffCallback',hObject,eventdata,guidata(hObject)), 'Label','Parykinti pelei artimiausius takus', 'Tag','aktyvusis');
% anotacijos
%h51 = uimenu( 'Parent',handles.m_Rodymas, 'Callback',' ', 'OnCallback',@(hObject,eventdata)pop_RRI_perziura('anotacijos_OnCallback',hObject,eventdata,guidata(hObject)), 'OffCallback',@(hObject,eventdata)pop_RRI_perziura('anotacijos_OffCallback',hObject,eventdata,guidata(hObject)), 'Label','Rodyti informacij apie artimiausius takus ties pele', 'Tag','anotacijos');
% Artinti2
...
% optimalusy
h58 = uimenu( 'Parent',handles.m_Rodymas, 'Callback',@(hObject,eventdata)pop_RRI_perziura('optimalus_rodymasy',hObject,eventdata,guidata(hObject)), 'Label','Optimalus auktis', 'Tag','optimalusy');
% optimalusx
...
%  ankste
h60 = uimenu( 'Parent',handles.m_Eiti, 'Callback',@(hObject,eventdata)pop_RRI_perziura('eiti_i_ClickedCallback',hObject,eventdata,guidata(hObject)), 'Label','okti  laik...', 'Tag','eiti_i');
% prie ankstesnio paymto
h61 = uimenu( 'Parent',handles.m_Eiti, 'Callback',@(hObject,eventdata)pop_RRI_perziura('eiti_pazym_atgal_ClickedCallback',hObject,eventdata,guidata(hObject)), 'Separator','on', 'Label','Prie ankstesnio paymto', 'Tag','eiti_pazym_atgal');
% prie tolesnio paymto
h62 = uimenu( 'Parent',handles.m_Eiti, 'Callback',@(hObject,eventdata)pop_RRI_perziura('eiti_pazym_tolyn_ClickedCallback',hObject,eventdata,guidata(hObject)), 'Label','Prie tolesnio paymto', 'Tag','eiti_pazym_tolyn');
...
if and(~isempty(handles.EKG),...
    ...
    % % Galjome pieti su plot  bettada linijas braukia Brush veiksena, jas gali itrinti
...
try 
    ...
    catch; ribos=[0 1500]; % Jei pirm kart
    end;
    try    RRI=get(handles.RRI_tsk,'YData'); % Jei atnaujinama
    catch; RRI=handles.RRI; % Jei pirm kart
    end;
...
% Kai mogus ranka itrina takus, gali bti,
% kad linijinis ir takinis grafikas truput skirsis
...
if iscell(RRI_);
    ...
    for i=1:length(Laikai__);
            ...
            if ri1>length(RRI1); warning('kaas negerai'); disp('ri1='); disp(ri1); ri1=ri1(ri1<=length(RRI1)); end;
            if ri2>length(RRI2); warning('kaas negerai'); disp('ri2='); disp(ri2); ri2=ri2(ri2<=length(RRI2)); end;
            if ri3>length(RRI3); warning('kaas negerai'); disp('ri3='); disp(ri3); ri3=ri3(ri3<=length(RRI3)); end;
            ...
            if length(r1)>1; warning('kaas negerai'); disp('r1='); disp(r1); r1=unique(r1); if length(r1)>1; r1=r1(r1>0); end; end;
            if length(r2)>1; warning('kaas negerai'); disp('r2='); disp(r2); r2=unique(r2); if length(r2)>1; r2=r2(r2>0); end; end;
            if length(r3)>1; warning('kaas negerai'); disp('r3='); disp(r3); r3=unique(r3); if length(r3)>1; r3=r3(r3>0); end; end;
        try % Dl visa pikto gaudykim klaid kertinje atnaujinimo vietoje
...
% Grafikai
handles.RRI=RRI; handles.Laikai=Laikai; % iedu nra tikri handles, t.y. tai ne objektai
...
disp(sprintf('x=%.4f; nx=%.4f; nx-x=%.4f; leidiama paklaida=%.1f ms', x_sek,nx_sek,nx_sek-x_sek,plotis_sek*1000));
...
for h=elementai; % saugiau eiti per kiekvien linij atskirai dl galim netikt naudotojo pakeitim po atnaujinimo
    dx=get(h,'XData');
    dy=get(h,'YData');
    i=max(find(dx < x_sek));
...
%     end;
    if min(abs(dx-x_sek))>0.005 % Paklaidos ribos
        if i>0;
            dxi=dx(i);
            if isnan(dy(i)); % virtualus prie terpiamj
                dyi=NaN ;
                dyj=0;
            ...
            elseif i == length(dx); % bus paskutinis
                dyi=dy(i); % ankstenio paskutinio nekeiiam
                dyj= (x_sek-dx(i)) *1000;
            else % per vidury  reikia siterpti tarp dviej gretim
                dyi= (x_sek-dx(i)) *1000;
    ...
    elseif ~perspeta;
        msg=sprintf(['\n ' ...
            'Bandote kurti alia kito pernelyg artimo tako! \n' ...
            'Tam, kad galtumte kurti tak pasirinktoje vietoje, \n' ...
            'arba priartinkite norim viet (leisima kurti takus su maesne paklaida), \n' ...
            'arba itrinkite sen tak (jei ir taip pakankamai ididinote norim srit).\n']);
...
switch ck
    case 'escape'
        uiresume(handles.figure1); % = ieiti i programos
        % pushbutton_atstatyti_Callback(hObject, eventdata, handles); = nuo pradi
    case 'enter'
        pushbutton_OK_Callback(hObject, eventdata, handles);
    case 'space'
        pushbutton_atnaujinti_Callback(hObject, eventdata, handles);
...
if ismember('control',modifiers);
    ...
    if eventdata.VerticalScrollCount * (1.5-asisN) > 0 % inversija Y aiai
        lim_dbr=get(handles.axes_rri,[asisR 'lim']);
        lim_max=get(handles.scrollHandles(asisN),[asisR 'lim']);
        lim_plt=(lim_dbr(2)-lim_dbr(1));
        lim_nj=min(lim_dbr(2) + lim_plt * 0.2, lim_max(2) + lim_plt * 0.2);
...
%Senas_EKG=handles.EKG; % Tyia leisti palikti sen EKG, jei yra tiksls R laikai
...
try
    ...
    if ekg_apversta(EKG,EKG_Hz,0); EKG=-EKG; end;
    handles.EKG=EKG;
    handles.EKG_laikai=[EKG_laikai / 1000]'; % sekundmis
    handles.EKG_Hz=EKG_Hz;
    zmkls=ismember(iv_tipai,ivyk_kit);
    handles.zmkl_lks=iv_latenc(zmkls) / 1000; % sekundmis
...
QRS_algoritmai={...
    ...
    'Suppappola-Sun (1994), i ECGlab2.0' ...
...
[f,p,i]=uigetfile({...
    ...
    '*.cnt;*.CNT' '*.CNT - ASA Lab'; ...
    '*'           '*';...
    }, 'Pasirinkite importuotin rinkmen');
if or(isempty(f),f==0) ; 
    %susildyk(hObject, eventdata, handles);
    return ; 
end;
...
function apie
msg=sprintf([...
    'Elektrokardiogramos ir R-R interval periros ir taisymo programl\n\n' ...
    'Skirta QRS aptikimui EKG rae, pasiruoimui tolesnei irdies ritmo kintamumo (variabilumo) analizei (pvz., su KubiosHRV).\n' ...
...
a=inputdlg('okti  laik (sekundmis):',mfilename,1,{num2str(xvid)});
if ~isempty(a);
    if ~isempty(a{1});
        t=str2num(a{1});
