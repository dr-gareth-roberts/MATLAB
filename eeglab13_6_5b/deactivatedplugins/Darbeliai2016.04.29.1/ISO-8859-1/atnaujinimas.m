...
% atnaujinimas()
%
% Lietuvikai: Parsisti ir diegti EEGLAB papildin
% NAUDOJIMAS: 
...
%                    yra diegta (sena) papildinio versija
%
%
...
% Btinai udaryk Darbeli langus
...
try
    disp('Parsiuniame archyv...');
    [filestr,status] = urlwrite(url,fullfile(path_new,[ name '.zip' ])) ;    
catch err ;
    disp(err.message) ;
    disp(char(['Nepavyko parsisisti' url [{}] 'Galbt neturite teiss rayti  aplank ' path_new ]));
    try   
...
% Jei pavyko parsisti, bandyk ipakuoti
if status == 1 ;
    try
        d1=dir(path_new); subdrs1={d1([d1.isdir]).name}; % poaplankiai prie ipakavim
        unzip(filestr,path_new);
        delete(filestr);
        d2=dir(path_new); subdrs2={d2([d2.isdir]).name}; % poaplankiai ipakavus
        ...
        disp(char(['Parsiuntme ir diegme papildin!' [{}] ' ' ]));
        try
           for fpi=1:length(files_to_preserve);
              fp=files_to_preserve{fpi};
              try
        ...
        disp(err.message);
        disp(char(['Parsiuntme ' filestr [{}] 'Bet nepavyko ipakuoti  ' path_new ]));
        try
            if strcmp(path_old(end),filesep);
                path_old=path_old(1:end-1);
            end ;
    ...
    disp(char(['Nepavyko parsisti ' url [{}] 'Galbt nra interneto ryio.']));
    try   
        if strcmp(path_old(end),filesep);
            path_old=path_old(1:end-1);
        end ;
