...
% licencijos kopija; jeia ne - raykite Free Software Foundation, Inc., 59
...
function anotObj=RRI_perziuros_anotacija_prideti(hFig,cAx)
    ...
    if str2num(V(1:end-1)) < 2015; 
        try delete(anotObj); catch; end; 
        anotObj=[];
        FaceAlpha=0; % tekstas nepermatomas ir taip, papildomo staiakampio pieimas gadina vaizd
        %return;
    else
    ...
    % Numatytas tekstas, jei nebt nubraiyt tak
    str={sprintf('%.0f ms\n%.3f s',axesCP(1,2),axesCP(1,1))};
    
    % Rasti artimiausi QRS ymel
    ...
    if ~isempty(clds);
        ...
        for ci=1:length(clds); c=clds(ci);
            % Apskaiiuoti atstum iki kiekvieno grafiko tako
        ...
        % % Naujas ymeklis, bet tik vienam - artimiausiam - takui 
        ...
        % Naujas ymeklis visiems takams su tuo paiu x
        ...
        % Urao parinkimas priklausomai nuo atstumo
        if ~isempty(point_h);
            if point_d < 20
