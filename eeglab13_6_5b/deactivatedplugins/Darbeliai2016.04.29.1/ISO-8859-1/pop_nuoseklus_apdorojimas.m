...
% Kurias parinktis jungti vos paleidus
...
if exist('pop_cleanline.m','file') == 2;
    ...
    %set(handles.checkbox_filtr_tinklo,'TooltipString', 'Reikia diegti Cleanline paket i http://www.nitrc.org/projects/cleanline/ . Kakodl bent iuo metu (2014 m. liepa) neveiks is priedas diegtas per paio eeglab meniu' ) ;
end;
...
%set(handles.uipanel6,'Title', ['Duomen apdorojimo funkcijos']);
set(handles.text_darbas,'String',' ');
...
% Nuostat siminimas, jei yra Darbeliai_config.mat
function_dir=regexprep(mfilename('fullpath'),[ mfilename '$'], '' );
try
    load(fullfile(Tikras_Kelias(fullfile(function_dir,'..')),'Darbeliai_config.mat'));
...
% Jei pasirinkta atlikti MARA - tai rezultat apibendrinim rayti  failiuk
...
for i=1:Pasirinktu_failu_N;
    ...
    if ~isempty(EEG);
        ...
        if get(handles.checkbox_kanalu_padetis,'Value') == 1 ;
            ...
            if and(~and(get(handles.radiobutton7,'Value') == 1, DarboPorcijaAtlikta > 0), and(EEG.nbchan > 0, and(~isempty(EEG.data), EEG.pnts>1)));
                ...
                try
                    ...
                    % Paalinti takus i kanal pavadinim
        ...
        % Atmesti triukmingas atkarples (pagal amplitud)
        ...
        % Atmesti triukmingas atkarples (pagal dani spektr)
        Darbo_apibudinimas=lokaliz('Rejecting noisy intervals...');
        ...
        % Atmesti triukmingus kanalus (auto)
        ...
        if get(handles.checkbox_perziureti,'Value') == 1 ;
            ...
            if and(~and(get(handles.radiobutton7,'Value') == 1, DarboPorcijaAtlikta > 0), and(EEG.nbchan > 0, and(~isempty(EEG.data), EEG.pnts>1)));
                ...
                try
                    ...
                    %f = warndlg(sprintf('Jums atvrme lang, kuriame silome perirti artefaktus. Nepamirkite patvirtinimui nuspausti Reject mygtuko. Klausiant kur saugoi fail, tiesiog spauskite OK. '), 'Dmesio!');
                    %disp('Dmesio! Kai nuspausite mygtuk, usivers langas, kuriame silome perirti artefaktus.');
                    %drawnow     % Necessary to print the message
                    %waitfor(f);
        ...
        if get(handles.checkbox_atmesk_iki2s,'Value') == 1 ;
            ...
            if and(~and(get(handles.radiobutton7,'Value') == 1, DarboPorcijaAtlikta > 0), and(EEG.nbchan > 0, and(~isempty(EEG.data), EEG.pnts>1)));
                ...
                try
                    ...
                    disp( [ 'Buvo bendra trukm: ' num2str(EEG.times(end)/1000 ) ]);
                    ...
                    disp( [ 'Nauja trukm: ' num2str(EEG.times(end)/1000 )]);
        ...
        % Atrinkti kanalus, interpoliuojant trkstamus
        ...
        if get(handles.checkbox_epoch,'Value') == 1 ;
            ...
            if and(~and(get(handles.radiobutton7,'Value') == 1, DarboPorcijaAtlikta > 0), and(EEG.nbchan > 0, and(~isempty(EEG.data), EEG.pnts>1)));
                ...
                try
                    ...
                    disp(['pagal vykius (' num2str(length(Epochuoti_pagal_stimulus)) '): ' ]);	
...
if or(~and(get(handles.radiobutton7,'Value') == 1, PaskutinioIssaugotoDarboNr <  DarboNr ),...
   ...
   atnaujinti_eeglab=true;
    ...
    % Grinti senuosius EEGLAB kintamuosius ir atnaujinti lang
    ...
    % Udaryk fail, jei MARA apibendrinimai buvo raomi
    if get(handles.checkbox_MARA,'Value') == 1 ;
        fclose(fid);
    end;
...
try
    ...
    button3 = questdlg([' ' KELIAS_siulomas ' ' ] , ...
        'Neradome aplanko', ...
        'Ataukti', 'Sukurti aplank', 'Sukurti aplank');
    if strcmp(button3,'Sukurti aplank')
        try
...
% rae nesantys kanalai interpoliuojami.
...
%vis tik nepaisyti pradinis_pasirinkimas, jei netuias ankstesnis pasirinkimas
