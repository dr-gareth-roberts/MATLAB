...
%simink prie funkcij vykdym buvus keli; netrukus bandysime laikinai pakeisti keli
Kelias_dabar=pwd;
...
% Pabandyk nustatyti keli pagal parametrus
...
% Sugrk  keli prie ios funkcijos atvrim
cd(Kelias_dabar);

% Patikrink keli duomen isaugojimui
...
% reikms numatytosios
set(handles.checkbox57,'Value',0);%Perira
set(handles.checkbox58,'Enable','on'); % Fail vidurkis
set(handles.checkbox58,'Visible','on');
set(handles.checkbox59,'Value',1);%Kanal vidurkis
  ...
  if ~isempty(g(1).mode);
    ...
    if or(ismember(g(1).mode,{'c' 'confirm'}),...
      and(ismember(g(1).mode,{'t' 'try' 'tryexec' 'tryforce'}),~agv)    );
        uiwait(handles.figure1); % UIRESUME bus vykdius uduotis
    end;
...
f = warndlg(sprintf('Ar tikrai galime eiti prie kito darbo?'), 'Dmesio!');
disp('Ar tikrai perirjote duomenis? Eisime prie kit darb.');
...
% Uverti lang, jei nurodyta parinktyse
try    
...
if isempty(FAILAI);
    ...
    %set(handles.uipanel5,'Title','Apdorotini failai: nra');
else
    if strcmp(get(handles.edit_failu_filtras2,'Style'),'edit') ;
        ...
        try
            ...
            Pranesk_apie_klaida(err, 'Apdorot duomen pasirinkimas', '')
        end;
    end;
...
%Neleisti spausti Nuostat meniu!
...
set(handles.checkbox59,'Enable','off'); %Kanal vid
set(handles.checkbox58,'Enable','off'); %Fail vid
set(handles.checkbox57,'Enable','off'); %Perira
set(handles.checkbox60,'Enable','off'); %Legenda
set(handles.checkbox72,'Enable','off'); %Y ais
...
set(handles.checkbox59,'Enable','on'); %Kanal vid
set(handles.checkbox58,'Enable','on'); %Fail vid
popupmenu11_Callback(hObject, eventdata, handles)
if get(handles.checkbox57, 'Value');
    set(handles.checkbox60,'Enable','on'); %Legenda
    set(handles.checkbox72,'Enable','on'); %Y ais
else    
    set(handles.checkbox60,'Enable','off'); %Legenda
    set(handles.checkbox72,'Enable','off'); %Y ais
end;
...
set(handles.checkbox_pabaigus_atverti,'Visible','on');
set(handles.checkbox57,'Enable','on'); %Perira

%Vidinis atlikt darb skaitliukas
...
disp(' ');
NaujaAntraste=[ num2str(DarboNr + str2num(get(handles.text_atlikta_darbu,'String'))) ' darb., ' num2str(i) '/' num2str(Pasirinktu_failu_N) ' r.'];
disp(NaujaAntraste);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Galima naudoti nukreipiant  seniau apraytas funkcijas, taiau tuomet nebus atsivelgiama  pasirinktus kanalus ir vykius
% 
...
%     warndlg('Programl neibaigta (beta versija). Ji dar neatrenka pasirinkt kanal ir vyki eksportuodama, bet tai daro perirint duomenis.');
% else       
%     Reikalingi_kanalai={};
% end;
% 
...
if ~isempty(Pasirinkti_kanalai);
    ...
    Reikalingi_kanalai={}; %Nebus keiiamas darb eigoje
    Reikalingi_kanalai_sukaupti={}; % bus keiiamas darb eigoje
end;
Eksp=get(handles.checkbox69,'Value');
...
% ALLEEGTMP nevisai atitiks ALLEEG, nes ALLEEGTMP kuriamas jau atrinkus vykius, turi 'erp_data'
...
for i=1:Pasirinktu_failu_N;
    ...
    try
        ...
        % kelti
        Darbo_eigos_busena(handles, 'keliama...', DarboNr, i, Pasirinktu_failu_N);
        ...
        if ~isempty(di);
            ...
            if ~isempty(Epochuoti_pagal_stimulus_);
                    ...
                    for i_epoch=1:length(Epochuoti_pagal_stimulus_) ;
                        try
                            orig_epoch=Epochuoti_pagal_stimulus_{i_epoch}; % teksto eilutms
                        catch
                            orig_epoch=Epochuoti_pagal_stimulus_(i_epoch); % skaiiams
                        end;
...
%             legendoje(li,2)=ALLEEG_(end).chans;           
        ...
        if and(Ar_eksportuoti_savybes,~get(handles.checkbox58, 'Value'));
            ...
            DarboNr=1;
            Darbo_eigos_busena(handles, 'ERP savybs...', DarboNr, i, Pasirinktu_failu_N);
    ...
    %set(handles.uipanel6,'Title', ['Duomen apdorojimas: ' num2str(1 + DarboNr + str2num(get(handles.text_atlikta_darbu,'String'))) ' darb., ' num2str(i) '/' num2str(Pasirinktu_failu_N) ' r.']);
    ...
    %set(handles.uipanel6,'Title', ['Duomen apdorojimas: ' num2str(0 + DarboNr + str2num(get(handles.text_atlikta_darbu,'String'))) ' darb., ' num2str(i) '/' num2str(Pasirinktu_failu_N) ' r.']);
    ...
    if get(handles.radiobutton6,'Value') == 1;
        ...
        %Galbt naudotojas nori nutraukti anksiau
...
%% ERP savybi surinkimas, kai praoma vidurkinti
...
if get(handles.checkbox58, 'Value');
  if and(Ar_eksportuoti_savybes,get(handles.checkbox58, 'Value'));
            ...
            DarboNr=1;
            Darbo_eigos_busena(handles, 'ERP savybs...', DarboNr, 1, 1);
...
%% ERP savybi eksportavimas

if and(Ar_eksportuoti_savybes,~isempty(ALLEEG_(1).file));
    
    DarboNr=DarboNr+1;
    Darbo_eigos_busena(handles, 'ERP savybi eksportavimas...', DarboNr, i, Pasirinktu_failu_N);
    ...
    lentele{1,3}=lentele{1,1}; lentele{1,3}(2:lenteles_dydis_y,2)={[lokaliz('mean_amplitude_short') ', microV']};%vidutin amplitud
    lentele{2,2}=lentele{1,1}; lentele{2,2}(2:lenteles_dydis_y,2)={[lokaliz('half_area_time') ', ms']};%laikas ties pusploiu
    lentele{2,3}=lentele{1,1}; lentele{2,3}(2:lenteles_dydis_y,2)={[lokaliz('half_area_amplitude') ' ampl., microV']};%amplitud ties pusploiu
    ...
    try
        switch get(handles.popupmenu9,'Value')
            ...
            case 2
                %%
                dok=[regexprep(dok,'.txt$','') '.txt']; % visada .txt galn
                dok_id=fopen(dok,'w');
                %size(Excel_lentele)
...
%% Eksportuoti paius ERP duomenis, o ne savybes
...
if and(~isempty(ALLEEG_(1).file),get(handles.checkbox69,'Value'));
    ...
    if get(handles.popupmenu10,'Value')==1;
        ...
        DarboNr=DarboNr+1;
        Darbo_eigos_busena(handles, 'Eksportuoti  RAGU...', DarboNr, length(ALLEEG_), length(ALLEEG_));
    ...
    if get(handles.popupmenu10,'Value')==2;
        ...
        %Darbo_eigos_busena(handles, 'Eksportuoti  TXT...', DarboNr, 0, length(ALLEEG_));
        
        for eeg_i=1:length(ALLEEG_);
            try
                Darbo_eigos_busena(handles, 'Eksportuoti  TXT...', DarboNr, eeg_i, length(ALLEEG_));
    ...
    if get(handles.popupmenu10,'Value')==3;
        ...
        for eeg_i=1:length(ALLEEG_);
            try
                Darbo_eigos_busena(handles, 'Eksportuoti  Excel...', DarboNr, eeg_i, length(ALLEEG_));
                EEGTMP=ALLEEG_(eeg_i);
...
% Jei nra taip, kad pirmenyb eiti per darbus, o surasta daug darb
% t.y. arba pirmenyb eiti vienu rau per visus darbus
%      arba pirmenyb eiti per darbus, bet dar liko atliktin darb
%
% ARBA
%     pirmenyb eiti per darbus, o naudotojas prao baigti anksiau
%
if or(~and(get(handles.radiobutton7,'Value') == 1, PaskutinioIssaugotoDarboNr <  DarboNr ),...
    ...
    disp([ 'Buvo ' num2str(DarboNr) ' darbai(-as,-), o atliktas tik vienas  suksimas kitas darbas.']);
...
disp(' ');
disp('Darb nutrauk pats naudotojas');
susildyk(hObject, eventdata, handles);
...
if ~isempty(findobj('-regexp','name',mfilename)) ;
    if and(strcmp(get(handles.checkbox_baigti_anksciau,'Visible'),'on'), ...
        ...
        disp(' '); disp('Naudotojas priverstinai udaro lang!');
        set(handles.checkbox_baigti_anksciau,'Value',1);
        %set(handles.checkbox_baigti_anksciau,'Visible','on');
        checkbox_baigti_anksciau_Callback(hObject, eventdata, handles);
    elseif strcmp(get(handles.pushbutton4,'Enable'),'on') ;        
        ...
        switch button1
            case lokaliz('Close window')
                ...
                button2 = 'Tik uverti lang';
                
                
                % Neklausti
                if 1 < 0 ;
                    button2 = questdlg(['Jei per klaid nuspaudte uvrimo mygtuk, ' ...
                        'spauskite Tsti kaip buvo. ' ...
                        'Jei spausite Tik uverti lang, gali sutrikti ios ' ...
                        'programos darbai. Jei norite nutraukti ios programls ' ...
                        'darb, spauskite Visai nutraukti' ] , ...
                        'Darb stabdymas', ...
                        'Uveriant generuoti klaid', 'Tik uverti lang', lokaliz('Continue as is'), ...
                        'Tik uverti lang');
                end;
                
                switch button2
                    case 'Uveriant generuoti klaid'
                        delete(hObject);
                        error('Darb nutrauk naudotojas');
                        %    case 'Grti  parinktis'
                        %        %error('Darb nutrauk naudotojas');
                        %        pop_ERP_savybes;
                    case 'Tik uverti lang'
                        delete(hObject);
                        disp('Lang naudotojas uvr ');
                    case lokaliz('Allow change options')
                        disp('Naudotojas papra atitirpdyti parinktis');
                        susildyk(hObject, eventdata, handles);
...
% Mastelis grafiko x aiai.
% Jei mast=0.001 - tai sekundmis, jei mast=1 - sekundmis
mast=1;         
            ...
            if isequal(ALLEEG_.times);
                ...
                switch get(handles.popupmenu11,'Value')
                    case 1
                        ...
                        grpN=length(grpsar);
                        grpnar{1}=1:length(ALLEEG_);
                        rodyti_ypatingas_grupes=0;  % iimtis visuotiniam vidurkinimui
                    case 2
                        grpsar=unique({ALLEEG_.subject});
                        grpN=length(grpsar);
                        for grpid=1:grpN;
    ...
    if isempty(ALLEEG_(1).file);
        ...
        if isempty(netusti);
            ...
            elseif and(get(handles.checkbox58,'Value'),strcmp(get(handles.checkbox58,'Enable'),'on'));
                    ...
                    %else
                    %    warning('Nesutampa duomen laikai');
                    %    %set(handles.checkbox58,'Value',0);
                    %    %drawnow;
                    %end;
            end;
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
i=get(handles.edit1,'String'); % klimo
...
%a=inputdlg({'Rinkmenos priesaga prie atvejo:'},'Lentels klimas',1,{'.set'});
%Priesaga=a{1};
Priesaga='';
try
...
msgbox('Pavyko kelti lentel!','Pavyko!');
ERP_perziura(hObject, eventdata, handles);
...
switch raktas;
    ...
    otherwise
        ...
        % visumin paveiksl spausdinsime dukart: mastelio parinkimui ir pg. parinktis
        set(handles.checkbox58, 'Value',0);
        % rakto_nariai apibrti toliau
end;
