...
if isempty(g.listOfTimes.EEG);
    error('Neradome tinkam EEG vyki');
end;
...
% Vienas kanalas gali turti kelis blokus, 
% jei raymas buvo sustabdytas, o po to toliau rainta
...
elseif bloku_N > 1;
    ...
    if isempty(Potencialiai_tinkami_blokai);
        error('Yra bendr skaitini vyki, bet jie nesutampa!');
    end;
    ...
    error('LabChart rae nra skaiiais ukoduot vyki!');
end;
...
if isempty(g.listOfTimes.LabChart);
    error('Neradome tinkam LabChart vyki');
end;
...
if g.Netvarkingi_poslinkiai ;
    ...
    if sqrt( g.Tarpai_tarp_ivykiu.sumu_skirtumas ^ 2) <= 2 * g.Leidziama_paklaida ;
        % Ibandykim paprasta toki atveji (3m_I_1):
