...
% drb_meniu - bendr meniu sukrimas Darbeli languose 
%
    ...
    case {'pop_pervadinimas'}
       ...
       %flt_show = get(handles.edit_failu_filtras1,'String');
        ...
        if ~isempty(files_sN) && ~isequal(files_sN,0);
            ...
            ar_filtr = 0 ;%strcmp(get(handles.edit_filtras,'Style'),'edit') #FIXME: pop_pervadinime fail rankinis ymjimas neperjungia stiliaus
        else
            ar_filtr = 1;
        end;
...
%handles.meniu_nuostatos_irasyti = uimenu(handles.meniu_nuostatos,'Label','rayti');
...
% Katalog atverti operacins sistemos fail naryklje
