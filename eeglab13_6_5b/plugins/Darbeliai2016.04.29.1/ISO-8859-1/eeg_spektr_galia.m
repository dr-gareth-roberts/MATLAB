...
%
%% Gauna pasirinkt fail spektrin galios tank
%
...
%% Apraykime kintamuosius
analizuoti_pavadinima=0;
%AR_GRAFIKAS='off';
if analizuoti_pavadinima;
    % Svarbu nurodyti failo pavadinimo schem!
    %DUOMENYS.VISU.Pavadinimo_schema='s%d%c_%s';
    DUOMENYS.VISU.Pavadinimo_schema='%s';
end;
% Pirmojo intervalo atvilgiu bus skaiiuojama santykin galia
%DUOMENYS.VISU.Dazniu_sritys = { [1 45] [8 12] [8 13] ; };
% Turi atitikti dani srii kiek
%DUOMENYS.VISU.Dazniu_sriciu_pavadinimai = { 'visas' 'alfa8_12' 'alfa8_13' ;} ;
% Dmesio! Raomi ne daniai, o aukiau ivardint dani indeksai, t.y.
% [4 3] reikia, kad bus dalinama ketvirto danio absoliuti reikm i treio danio absoliuios reikms
...
% Skyriklis nurodant santykin gali
skyriklis='/';
...
%%
% Staiakampje (Excel) lentelje { 'Pavad' x y ; ... }.
% ia tik 48 kanalai:
...
%% Jei reikt interpoliuoti kanalus  informacija interpoliavimui
...
% tikrinimas dl kanal tvarkos priskyrimo
...
for i=1:NumberOfFiles ;
    ...
    try
        ...
        try
            ...
            % Jei nebuvo interpoliacijos, tada reikia naudoti sekani eilut, kad
            % nenult programa beiekodama nesamo kanalo
        ...
        if and(EEG.nbchan > 0, ~isempty(EEG.data));
            ...
            if analizuoti_pavadinima
                % Pakeitus DUOMENYS.VISU.Pavadinimo_schema ar duomen aplank, teks
                % i naujo aprayti informacijos atpainim pagal fail.
            ...
            if ~isequal(DUOMENYS.VISU.DAZNIAI, DUOMENYS.FAILO(i).DAZNIAI);
                warning(['Nesutampa EEG.srate su kit fail. ' File]);
            ...
            if isequal(DUOMENYS.VISU.KANALAI, DUOMENYS.FAILO(i).KANALAI);
                ...
                warning(['Nesutampa kanalai su kit fail. ' File]);
            end;
...
%% Spektrins galios tankio duomen apdorojimas - Galios skaiiavimas
    ...
    for kan=1:DUOMENYS.VISU.KANALU_N;
       ...
       if DUOMENYS.VISU.Statistika_Stjudento_absol_galiai.h(i,kan) == 1 ;
            ...
            elseif DUOMENYS.VISU.Statistika_Stjudento_absol_galiai_l.h(i,kan) == 1 ;
                ...
                disp(['Vidin klaida: i=' num2str(i) ', kan=' num2str(kan) ', turi bti ' galvoje_reiksme ', bet yra ' galvoje_reiksme2 ]);
            end ;
        ...
        if DUOMENYS.VISU.Statistika_Vilkoksono_absol_galiai.h(i,kan) == 1 ;
            ...
            elseif DUOMENYS.VISU.Statistika_Vilkoksono_absol_galiai_l.h(i,kan) == 1 ;
                ...
                disp(['Vidin klaida: i=' num2str(i) ', kan=' num2str(kan)  ]);
            end ;
...
if ~(isempty(find(ismember(Doc_tp, {'txt','TXT'})))) && ( Ar_reikia_galios_absoliucios || Ar_reikia_galios_santykines );
    ...
    %% Rezultat po Stjudento eksportavimas
    ...
    if and(0,DUOMENYS.VISU.Salygu_N == 2) ;
        ...
        %% Rezultat po Vilkoksono eksportavimas
    ...
    %% Spektrins galios eksportavimas  tekstin fail
...
if ~(isempty(find(ismember(Doc_tp, {'txt','TXT'})))) && ( Ar_reikia_spekro_absol || Ar_reikia_spekro_db );
    ...
    %% Spektro eksportavimas  tekstin fail
