...
%% Part 1 = load file, load chan locs, filter 1-30 Hz
    for cntfile = 1:nCntFiles
        ...
        EEG  = pop_binlister( EEG , 'BDF', '/media/DATAPART1/Gareth/flanker_bins2.txt', 'ExportEL', strcat('/media/DATAPART1/Gareth/FlankerSeq/Events/',char(loadCnt(1:8)),'_events.txt'), 'IndexEL',  1, 'SendEL2', 'EEG&Text', 'UpdateEEG', 'on', 'Voutput', 'EEG' ); 
...
%         EEG  = pop_artstep( EEG , 'Channel',  1:33, 'Flag',  1, 'Threshold',  115, 'Twindow', [ -200 996], 'Windowsize',  200, 'Windowstep',100);
        ...
        % export RTs
        values = pop_rt2text(ERP, 'eventlist',  1, 'filename', strcat('/media/DATAPART1/Gareth/FlankerSeq/RTs/',char(loadCnt(1:8)),'_rts.txt'), 'header', 'on', 'listformat', 'basic' );
    end;
%% part 2
% get everything ready (paths, eeglab, current directory, etc.)
...
% get the files in the directory and count how many there are
 ...
 nSetFiles=length(setFilesinDir);
    ...
    for setfile = 1:nSetFiles
        ...
        % export RTs
        values = pop_rt2text(ERP, 'eventlist',  1, 'filename', strcat('/Users/Gareth/Documents/Academic Work/Flanker2/',char(loadSet(1:8)),'_rts.txt'), 'header', 'on', 'listformat', 'basic' );
    end;
