 ...
 %% Separate epochs
    ...
    nSetFiles=length(setFilesinDir);   
     ...
     for setfile = 1:nSetFiles
            ...
            erpFileName = strcat(loadSet(1:4),'_standards');
            EEG = pop_saveset( EEG,'filename',erpFileName,'filepath',subjectDir);
            pop_export(EEG,strcat(resultspath,erpFileName,'_erp'),'precision',4);
            EEG = pop_loadset('filename',setLocation);
            
            % create target epochs
            EEG = pop_epoch( EEG, {  'B2(3)'  }, EPOCH_LENGTH, 'newname', 'target', 'epochinfo', 'yes');
            EEG = pop_rmbase( EEG, BASELINE_CORRCTION);
            erpFileName = strcat(loadSet(1:4),'_targets');
            EEG = pop_saveset( EEG,'filename',erpFileName,'filepath',subjectDir);
            pop_export(EEG,strcat(resultspath,erpFileName,'_erp'),'precision', 4);
            EEG = pop_loadset('filename',setLocation);
            ...
            EEG = pop_rmbase( EEG, BASELINE_CORRCTION);
            erpFileName = strcat(loadSet(1:4),'_novels');
            EEG = pop_saveset( EEG,'filename',erpFileName,'filepath',subjectDir);
            pop_export(EEG,strcat(resultspath,erpFileName,'_erp'),'precision',4);
            EEG = pop_loadset('filename',setLocation);
     end;
...
[STUDY ALLEEG] = std_editset( STUDY, ALLEEG, 'name', 'AO task', ...
     ...
     'updatedat','off', 'filename', fullfile(basedir, 'AO_STA.study'));
